...
public class CharacterControllerStateMachine : BaseStateMachine<CharacterState>, IDamageable
{
    ...
    [field: SerializeField]
    protected GameObject m_punchHitbox;
    ...
    public void OnEnableAttackHitbox(bool isEnabled = true)
    {
        Debug.Log(gameObject.name + ": Attack hitbox is enable = " + isEnabled);
        m_punchHitbox?.SetActive(isEnabled);
    }

    public void Step()
    {
        if (CurrentRelativeVelocity.y > 0.1f || CurrentRelativeVelocity.x > 0.1f || CurrentRelativeVelocity.y < -0.1f || CurrentRelativeVelocity.x < -0.1f)
